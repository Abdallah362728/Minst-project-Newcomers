...
private:
    // Weights and Biases
    std::array<float, inputs * hidden_neurons> m_inputLayerWeights;
    ...
    std::array<float, inputs * hidden_neurons> m_inputLayerWeightsDeltaCost;
...
public:
    ...
    const std::array<float, inputs * hidden_neurons> &GetHiddenLayerWeights() const { return m_inputLayerWeights; }
    ...
    NeuralNetwork()
    {
        ...
        for (float &f : m_inputLayerWeights)
            f = dist(e2);
    ...
    void Train(const MNISTData &trainingData, size_t miniBatchSize, float learningRate)
    {
        ...
        while (trainingIndex < trainingData.NumImages())
        {
            ...
            while (miniBatchIndex < miniBatchSize && trainingIndex < trainingData.NumImages())
            {
                ...
                for (size_t i = 0; i < m_inputLayerWeightsDeltaCost.size(); ++i)
                    m_miniBatchHiddenLayerWeightsDeltaCost[i] += m_inputLayerWeightsDeltaCost[i];
            ...
            for (size_t i = 0; i < m_inputLayerWeights.size(); ++i)
                m_inputLayerWeights[i] -= m_miniBatchHiddenLayerWeightsDeltaCost[i] * miniBatchLearningRate;
    ...
    uint8_t ForwardPass(const float *pixels, uint8_t correctLabel)
    {
        for (size_t neuronIndex = 0; neuronIndex < hidden_neurons; ++neuronIndex)
        {
            ...
            for (size_t inputIndex = 0; inputIndex < inputs; ++inputIndex)
                Z += pixels[inputIndex] * m_inputLayerWeights[HiddenLayerWeightIndex(inputIndex, neuronIndex)];
    ...
    void BackwardPass(const float *pixels, uint8_t correctLabel)
    {
        ...
        for (size_t neuronIndex = 0; neuronIndex < hidden_neurons; ++neuronIndex)
        {
            ...
            // Calculating deltaCost/deltaWeight for each weight going into the neuron
            for (size_t inputIndex = 0; inputIndex < inputs; ++inputIndex)
                m_inputLayerWeightsDeltaCost[HiddenLayerWeightIndex(inputIndex, neuronIndex)] = m_hiddenLayerBiasesDeltaCost[neuronIndex] * pixels[inputIndex];
        }
    }
};